...
public class Bank {
    ...
    public void displayMyAccounts(String ownerName) {
    	...
    	System.out.println("--------------------------------------------------------");
    	System.out.println("계좌번호\t\t현재 잔액\t적립기간");
    	// 배열에 저장된 Account객체의 주소값을 순서대로 가져와서 account참조변수에 대입한다.
    	for (Account account : database) {
    		// 참조변수 account에 대입된 주소값이 null이 아니면
    		if (account == null) {
    			break;
    		}
			// 참조변수 account로 참조되는 Account객체의 소유주이름을 조회한다.
			String accountOwnerName = account.getOwner();
			// 조회된 계좌의 소유주이름과 매개변수로 전달받은 소유주이름이 일치하면
			if (accountOwnerName.equals(ownerName)) {
				isExist = true;
				// account참조변수가 참조하는 Account객체의 계좌아이디, 잔액, 납입기간을 화면을 출력한다.
    	...
    	if (!isExist) {
    		System.out.println("### [완료] 개설된 계좌정보가 없습니다.");
    	} else {    		
    		System.out.println("### [완료] " + ownerName + "님이 개설한 모든 계좌정보가 출력되었습니다.");
    	}
    }
    ...
    public void displayAccountInfo(String id, int password) {
    	...
    	System.out.println("--------------------------------------------------------");
    	System.out.println("### [완료] 계좌 상세정보가 출력되었습니다.");
    }
    ...
    public void changePassword(String id, int oldPassword, int newPassword) {
        ...
        System.out.println("### [완료] 계좌 비밀번호가 변경되었습니다.");
    }
    ...
    public void deposit(String id, long amount) {
    	if (amount <= 0) {
    		System.out.println("### [오류] 입금액이 유효하지 않습니다.");
    		return;
    	}
        ...
        System.out.println("--------------------------------------------------------");
        System.out.println("### [완료] 입금이 완료되었습니다.");
    }
    ...
    public void withdraw(String id, int password, long amount) {
        if (amount <= 0) {
        	System.out.println("### [오류] 출금액이 유효하지 않습니다..");
        	return;
        }
        ...
        System.out.println("--------------------------------------------------------");
        System.out.println("### [완료] 출금이 완료되었습니다.");
    }
    ...
    public void transfer(String id, String otherId, int password, long amount) {
    	if (amount <= 0) {
    		System.out.println("### [오류] 이체금액이 유효하지 않습니다.");
    		return;
    	}
    	...
    	System.out.println("--------------------------------------------------------");
    	System.out.println("### ["+otherId+"] 계좌로 ["+amount+"]원이 이체되었습니다.");        
        System.out.println("### 이체 전 잔액 : " + prevBalance + "원");
        System.out.println("### 이체 후 잔액 : " + account.getBalance() + "원");
        System.out.println("--------------------------------------------------------");
        System.out.println("### [완료] 이체가 완료되었습니다.");
    }
    ...
    public void calculateCancellationRefund(String id, int password) {
    	...
    	long interest = (long)((interestRate/12)*depositPeriod*balance);
    	long cancellationRefund = balance + interest;
    	
    	System.out.println("--------------------------------------------------------");
    	System.out.println("### 현재 잔액: " + balance + "원");
    	System.out.println("### 총 이자: " + interest + "원");
    	System.out.println("### 예상 해지환급금: " + cancellationRefund + "원");
    	System.out.println("--------------------------------------------------------");
    	System.out.println("### [완료] 예상해지환급금 계산이 완료되었습니다.");
     ...
     * @param accId 계좌아이디
     * @return 계좌정보, 계좌아이디가 잘못되었을 경우 null값이 반환될 수 있다.
     */
    private Account getAccount(String accId) {
    	...
    	for (Account account : database) {
    		// account참조변수에 대입된값이 null이 아니고, account가 참조하는객체의 아이디와 비밀번호가 일치하면
    		if (account != null && account.getId().equals(accId)) {
    			// 현재 조회된  account 객체의 주소값을 foundAccount 참조변수에 대입한다.
